...
define("actions/businessHours", [
  ...
  const createIssueOutOfBusinessHours = () => {
    return (dispatch, getState) => {
      const {
        appState: {domain, tags, cif, metadata, fullPrivacyEnabled, developerSetLanguage},
        businessHoursViewState: {
          contactFormDetails: {attachmentsMeta, attachments, message, name, email},
          inBusinessHours
        }
      } = getState();
      ...
      upload({
        ...
        onFailure: (failureResponse) => {
          const responseAttachments = objectUtils.getIn(failureResponse, [
            "responseData",
            "data",
            "attachments"
          ]);
          if (responseAttachments && responseAttachments.length) {
            const attachmentsWithError = getAttachmentsErrorActions(responseAttachments);
