  ...
  const setStyles = (domain, helpshiftConfig, callbacks = {}) => {
    getCss({
      ...
      onEnd: () => {
        ...
        // More details - With SDK X GA release (May 2021), we are removing helpshiftConfig.uiConfig
        ...
        // This XHR hits a non-existent endpoint with helpshiftConfig as a param. We will then check
        ...
        // Use localstorage to not fire this request every time the page is loaded.
        if (helpshiftConfig && !lsHelpers.get(LS_KEYS.HS_CONFIG_LOGGED)) {
          commonHelpers.logHsConfig(domain, {
            hsConfig: JSON.stringify(helpshiftConfig)
          });
        }
      }
    });
