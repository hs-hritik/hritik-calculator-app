...
define("components/commons/fileInput", ["gunpowder/utils/classes"], function(classes) {
  "use strict";

  const DEFAULT_ACCEPT =
    ".zip, .rar, .tar, .gzip, .mp3, .mpeg, .wav, .ogg, .amr, .jpeg, " +
  ...
  return createReactClass({
    ...
    render() {
      ...
      if (infoText) {
        infoTextEl = <span className="hs-file-input__info-text">{infoText}</span>;
      }

      const primaryIconClasses = classes("hs-file-input__icon", iconClasses);

      const fileInputClasses = classes(labelClasses, "hs-file-input", {
