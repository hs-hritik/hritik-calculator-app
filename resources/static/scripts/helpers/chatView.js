...
define("helpers/chatView", [
...
], function(MESSAGE_CONSTANTS, chatViewConstants, appStateConstants, commonHelpers, schema) {
  "use strict";
  ...
  const NO_INPUT_VALIDATIONS_REQUIRED_TYPES = [
    USER_INPUT_TYPES.PILL_SELECT,
    USER_INPUT_TYPES.LIST_PICKER
  ];
  const {Input} = schema;
  ...
  const getProcessedUserInput = (config) => {
    ...
    const userInputType = getUserInputType(messageType, optionsInputType, optionsLen);
